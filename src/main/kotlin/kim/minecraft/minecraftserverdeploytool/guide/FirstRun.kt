    ...
    private fun deployServerCore() {
        when (tempSettings["Corename"]) {
            "Akarin" -> {
                ...
                AkarinDeploy(version, build, saveDir, fileName).runTask()
            }
            "Arclight" -> {
                ...
                print("请输入您欲部署的服务端核心保存文件名，留空即为原名称: ")
                val fileName = scanner.nextLine().takeIf { it.isNotEmpty() }
                ArclightDeploy(version, saveDir, fileName).runTask()
            }
            "CatServer" -> {
                print("请输入您欲部署到的目录，留空即为当前目录: ")
                val saveDir = scanner.nextLine().takeIf { it.isNotEmpty() } ?: ".\\"
                print("请输入您欲部署的服务端核心保存文件名，留空即为原名称: ")
                val fileName = scanner.nextLine().takeIf { it.isNotEmpty() }
                CatServerDeploy(saveDir, fileName).runTask()
            }
            "Contigo" -> {
                print("请输入您欲部署到的目录，留空即为当前目录: ")
                val saveDir = scanner.nextLine().takeIf { it.isNotEmpty() } ?: ".\\"
                print("请输入您欲部署的服务端核心保存文件名，留空即为原名称: ")
                val fileName = scanner.nextLine().takeIf { it.isNotEmpty() }
                ContigoDeploy(saveDir, fileName).runTask()
            }
            "CraftBukkit" -> {
                ...
                print("请输入您欲部署的服务端核心保存文件名，留空即为原名称: ")
                val fileName = scanner.nextLine().takeIf { it.isNotEmpty() }
                CraftBukkitDeploy(version, saveDir, fileName).runTask()
            }
            "Magma" -> {
                print("请输入您欲部署到的目录，留空即为当前目录: ")
                val saveDir = scanner.nextLine().takeIf { it.isNotEmpty() } ?: ".\\"
                print("请输入您欲部署的服务端核心保存文件名，留空即为原名称: ")
                val fileName = scanner.nextLine().takeIf { it.isNotEmpty() }
                MagmaDeploy(saveDir, fileName).runTask()
            }
            "Mohist" -> {
                print("请输入您欲部署到的目录，留空即为当前目录: ")
                val saveDir = scanner.nextLine().takeIf { it.isNotEmpty() } ?: ".\\"
                print("请输入您欲部署的服务端核心保存文件名，留空即为原名称: ")
                val fileName = scanner.nextLine().takeIf { it.isNotEmpty() }
                MohistDeploy(saveDir, fileName).runTask()
            }
            "Paper" -> {
                print("请输入您欲下载的服务端核心游戏版本，留空即为下载最新版本: ")
                val version = scanner.nextLine().takeIf { it.isNotEmpty() }
                ...
                PaperDeploy(version, build, saveDir, fileName, fastMode).runTask()
            }
            "Spigot" -> {
                print("请输入您欲下载的服务端核心游戏版本，留空即为下载最新版本: ")
                val version = scanner.nextLine().takeIf { it.isNotEmpty() }
                ...
                print("请输入您欲部署的服务端核心保存文件名，留空即为原名称: ")
                val fileName = scanner.nextLine().takeIf { it.isNotEmpty() }
                SpigotDeploy(version, saveDir, fileName).runTask()
            }
            "SpongeForge" -> {
            ...
            "Tuinity" -> {
                ...
                print("请输入您欲部署的服务端核心保存文件名，留空即为原名称: ")
                val fileName = scanner.nextLine().takeIf { it.isNotEmpty() }
                TuinityDeploy(build, saveDir, fileName).runTask()
            }
            "Uranium" -> {
                print("请输入您欲下载的服务端核心构建版本，留空即为下载最新构建:  ")
                val build = scanner.nextLine().takeIf { it.isNotEmpty() }
                ...
                print("请输入您欲部署的服务端核心保存文件名，留空即为原名称: ")
                val fileName = scanner.nextLine().takeIf { it.isNotEmpty() }
                UraniumDeploy(build, saveDir, fileName).runTask()
            }
            "Vanilla" -> {
                val normalVersion = "1.16.2"
                print("请输入您欲下载的服务端核心游戏版本，留空即为下载 $normalVersion 版本: ")
                ...
                VanillaDeploy(version, saveDir, fileName, fastMode).runTask()
            }
            "VanillaForge" -> {
                val normalVersion = "1.16.2"
                print("请输入您欲下载的服务端核心游戏版本，留空即为下载 $normalVersion 版本: ")
